...
// UpdateConditionFailure updates the status of the QueryConnector resource with a failure condition
...
func (r *QueryConnectorReconciler) UpdateConditionNoCredsFound(QueryConnector *v1alpha1.QueryConnector) {
	...
	// Create the new condition with the success status
	condition := globals.NewCondition(globals.ConditionReasonNoCredsFoundType, metav1.ConditionTrue,
